...
/*$('#add-date').change(function () {
 var date = $('#add-date');
 if(date.val().match(/^20\d\d-(0?[1-9]|1[0-2])-(0?[1-9]|[12][0-9]|3[01])$/)){
 var current = new Date();
 var year = date.val().split("-")[0];
 var month = date.val().split("-")[1];
 var day = date.val().split("-")[2];
 var info = $('#addDateInfo');
 var error = false;
 if(year < current.getFullYear()){
 info.text("Invalid year");
 }else if(month < (current.getMonth()+1)){
 info.text("Invalid month");
 }else if(month === current.getMonth() && day < current.getDate()){
 info.text("Invalid day");
 }
 if(info.text() !== ""){
 date.val(current.getFullYear()+"-"+current.getMonth()+"-"+current.getDate());
 info.removeClass('hidden');
 info.closest('.form-group').addClass('has-error');
 }else{
 info.text("");
 info.addClass('hidden');
 info.closest('.form-group').removeClass('has-error');
 if ( day == current.getDate() && month == current.getMonth() && year == current.getFullYear()){
 $('input[name=optionsRadios]').each(function (k) {
 if($(this).val() <= current.getHours()){
 $(this).attr("disabled", "");
 }
 ...
 $('input[name=optionsRadios]:disabled').attr('disabled', false);
 }
